<template>
  <div class="actualizar-parcial-estudiante-container">
    <h2>Actualizar Parcialmente Estudiante</h2>
    <p class="descripcion">Modifique solo los campos que desea actualizar. Los campos vacíos no se modificarán.</p>
    ...
    <form v-if="!cargando" @submit.prevent="actualizarParcialEstudiante" class="formulario">
      ...
      <div class="form-grupo">
        ...
        />
        <small class="valor-actual">Valor actual: {{ estudianteOriginal.nombre }}</small>
      </div>
      ...
      <div class="form-grupo">
        ...
        />
        <small class="valor-actual">Valor actual: {{ estudianteOriginal.apellido }}</small>
      </div>
      ...
      <div class="form-grupo">
        ...
        />
        <small class="valor-actual">Valor actual: {{ formatearFecha(estudianteOriginal.fechaNacimiento) }}</small>
      </div>
      ...
      <div class="form-grupo">
        ...
        <input 
          ...
          placeholder="Ingrese la provincia (opcional)"
        />
        <small class="valor-actual">Valor actual: {{ estudianteOriginal.provincia }}</small>
      </div>
      ...
      <div class="form-grupo">
        ...
        <small class="valor-actual">Valor actual: {{ estudianteOriginal.genero }}</small>
      </div>
...
export default {
  ...
  methods: {
    async cargarEstudiante() {
      ...
      try {
        ...
        if (data) {
          ...
          // No pre-cargar campos en el formulario de actualización parcial
          // El usuario debe ingresar solo lo que quiere cambiar
        } else {
    ...
    async actualizarParcialEstudiante() {
      ...
      try {
        const id = this.estudianteOriginal.id;
        ...
        if (this.estudiante.fechaNacimiento) body.fechaNacimiento = this.estudiante.fechaNacimiento + 'T00:00:00';
        if (this.estudiante.provincia) body.provincia = this.estudiante.provincia;
        if (this.estudiante.genero) body.genero = this.estudiante.genero;

        if (Object.keys(body).length === 0) {
          this.mensajeError = 'Debe modificar al menos un campo';
          this.actualizando = false;
          return;
        }

        console.log('ID del estudiante:', id);
        console.log('Datos a actualizar parcialmente:', body);

        const resultado = await actualizarParcialFachada(id, body);
        console.log('Resultado de actualización parcial:', resultado);
        
        this.mensajeExito = '¡Estudiante actualizado parcialmente exitosamente!';
...
<style scoped>
.actualizar-parcial-estudiante-container {
  display: flex;
...
.btn-actualizar-parcial:hover:not(:disabled) {
  background-color: rgb(7, 140, 100);
}

.btn-actualizar-parcial:disabled {
